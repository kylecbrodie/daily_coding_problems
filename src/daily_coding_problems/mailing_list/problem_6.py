...
class XORLinkedListNode:
	def __init__(
		self,
		val: Any,
		prev: 'XORLinkedListNode' = None,
		next: 'XORLinkedListNode' = None
	):
		self.val = val
...
class XORLinkedList:
	...
	def get(self, index: int) -> XORLinkedListNode:
		...
		return self._get(
			self.head if closer_to_head else self.tail,
			index if closer_to_head else self.count - 1 - index
		)

	def _get(
		self, starting_node: XORLinkedListNode, index: int
	) -> XORLinkedListNode:
		if index < 0 or index >= self.count:
			raise IndexError('index out of range')
		if index == 0:
