from itertools import chain, dropwhile, product, repeat
...
class Solution:
	...
	def perfectNumbers(self):
		...
		while True:
			...
			variable_digits_ranges = chain([range(1, 10)],
				repeat(range(10), num_digits - 2))
	...
	def nthPerfectNumber(self, n):
		...
		"""
		_, nth_perfect_number = next(
			dropwhile(
			lambda t: t[0] < n, enumerate(self.perfectNumbers(), start=1)
			)
		)
